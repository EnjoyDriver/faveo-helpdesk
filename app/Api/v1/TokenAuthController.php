...
class TokenAuthController extends Controller
{
    ...
    public function authenticate(Request $request)
    {
        ...
        try {
            if (!$token = Auth::guard('api')->setTTL(287971200)->attempt([
                $field => $usernameinput, 'password' => $password, 'active' => 1
            ])) {
            ...
            if (isset($user)) {
                ...
                if (isset($password_reset_table)) {
                    ...
                    $create_password_reset = DB::table('password_resets')->insert([
                        'email' => $user->email, 'token' => $code, 'created_at' => $date
                    ]);
                }
                ...
                $this->PhpMailController->sendmail($from = $this->PhpMailController->mailfrom('1', '0'),
                    ...
                    $template_variables = [
                        'user' => $user->user_name, 'email_address' => $user->email,
